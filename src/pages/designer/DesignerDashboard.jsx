...
import { apiGetDesignerDashboard } from '../../api/api';
...
const DesignerDashboard = ({ navigateTo }) => {
  ...
  useEffect(() => {
    apiGetDesignerDashboard().then(setData).catch(console.error);
  }, []);
  ...
  return (
    <div className="space-y-7 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <StatCard title="Designs Published" value={String(data.stats.designsPublished)} icon={<Icons.Design />} color="from-purple-500 to-purple-600" />
        <StatCard title="Pending Review" value={String(data.stats.pendingReview)} icon={<Icons.Log />} color="from-amber-500 to-amber-600" />
        <StatCard title="Products Using Designs" value={String(data.stats.linkedProducts)} icon={<Icons.Link />} color="from-blue-500 to-blue-600" />
